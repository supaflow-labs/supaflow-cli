...
describe('supaflow mcp stdio', () => {
  it('emits only valid JSON-RPC and lists 53 tools', async () => {
    ...
    expect(lines.map((l) => JSON.parse(l)).find((m) => m.id === 2)?.result?.tools?.length).toBe(53);
  }, 10000);
  ...
  it('raw-text tools return BOTH child streams (agent_logs must not drop stderr)', async () => {
    ...
    child.stdin.write(
      rpc({
        ...
        id: 5,
        method: 'tools/call',
        params: { name: 'agent_logs', arguments: {} },
      }),
    );
    await sleep(1500);
    child.kill();
  ...
  it('does NOT act as an MCP server when run as a plain CLI command', async () => {
    ...
    const child = spawn(process.execPath, [DIST, 'connectors', 'list'], {
      stdio: ['pipe', 'pipe', 'pipe'],
    });
    const out = collect(child.stdout);
    ...
    const sawServerResponse = out.value
      .split('\n')
      .filter((l) => l.trim())
      .some((l) => {
        try {
          const m = JSON.parse(l);
          return m.id === 9 && m.result?.tools;
        } catch {
          return false;
        }
      });
