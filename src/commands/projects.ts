...
export function registerProjectsCommands(program: Command): void {
  ...
  projects
    ...
    .option(
      '--type <type>',
      'Project type (pipeline|ingestion|transformation|activation)',
      'pipeline',
    )
    .action(
      withAuth(
        async (ctx: AuthContext, opts: { name: string; destination: string; type: string }) => {
          ...
          if (destError || !dest) {
            throw new CliError(
              `Destination datasource "${opts.destination}" not found.`,
              ErrorCode.NOT_FOUND,
            );
          }
    ...
    .action(
      withAuth(async (ctx: AuthContext, identifier: string, opts: { yes?: boolean }) => {
        ...
        let query = supabase
          ...
          .eq('workspace_id', workspaceId)
          .neq('state', 'deleted');
        query = isUuid(identifier) ? query.eq('id', identifier) : query.eq('api_name', identifier);

        const { data: project, error } = await query.limit(1).single();
        if (error || !project) {
          throw new CliError(`Project "${identifier}" not found.`, ErrorCode.NOT_FOUND);
        }

        const confirmed = await confirmDestructiveAction({
          yes: opts.yes,
          json: outputOptions.json,
          question: `Delete project "${project.name}" and its pipelines? This cannot be undone. [y/N] `,
          nonInteractiveMessage:
            'Refusing to delete a project without --yes in non-interactive mode.',
        });
        if (!confirmed) {
          console.log('Aborted.');
          return;
        ...
        await softDeleteEntity(supabase, 'project', project.id);

        if (outputOptions.json) {
          printOutput(formatGetJson({ id: project.id, name: project.name, state: 'deleted' }));
        } else {
